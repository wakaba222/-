...
export default async function CoachCustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: records }, { data: sales }] = await Promise.all([
    ...
    supabase
      .from('sales')
      .select('id, coach_id, customer_id, product_id, sold_on, amount, incentive_amount, acquisition_source, status, refund_amount, note, products(id, name, code, is_sales_score_target)')
      .eq('customer_id', id)
      .is('deleted_at', null)
