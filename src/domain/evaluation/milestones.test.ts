...
describe('売上の次の一歩', () => {
  it('次のアンカーまでの金額と、そこで増える点数を出す', () => {
    ...
    expect(milestone?.gap).toBe('155万円');
    ...
    expect(milestone?.reward).toBe(`評価が +${Math.round(gain * 10) / 10}点`);
  });
  ...
  it('示した金額に到達すると、実際に示した点数になる', () => {
    ...
    const gapMan = Number(milestone!.gap.replace(/[^\d]/g, ''));
...
describe('昇格の次の一歩', () => {
  it('残っている条件と、昇格して得られるものを出す', () => {
    ...
    // 条件の全文は下の一覧に出るため、ここでは残件数と一番近い1件だけを出す
  ...
  it('個別単価が既に次ランクの標準以上なら、単価が上がるとは言わない', () => {
    ...
    expect(milestone?.reward).toBe('P2 にランクアップ');
  ...
  it('個別単価が次ランクの標準より低ければ、その差額を示す', () => {
    ...
    expect(milestone?.reward).toBe('P2 にランクアップ・単価 +2,000円');
  });

  it('条件を満たしていればその旨を出す', () => {
    const milestone = promotionMilestone('P1', promotionOf({ status: 'CANDIDATE' }), RULES);
    expect(milestone?.gap).toBe('条件クリア');
